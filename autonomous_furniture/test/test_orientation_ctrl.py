...
def run_single_furniture_rotating():
    ...
    goal = ObjectPose(position=np.array([7, 5]), orientation=np.pi/2)  # , orientation = 1.6) Goal of the CuboidXd
    ...
    table_shape2 = CuboidXd(axes_length=[max_ax_len, min_ax_len],
                           center_position=np.array([4, 5]),
                           margin_absolut=0.6,
                           orientation=np.pi/2,
                           tail_effect=False,)
    goal2 = ObjectPose(position=np.array([4, 5]), orientation=np.pi/2)
