...
def compare_v2_vs_v1():
    ...
    for ll, nb_fur in enumerate(list_fur):
       ...
       for algo in list_algo:
            ...
            for jj, version in enumerate(list_vers):
                data = json.load(open(
                    f"new_seed_gen/distance_nb{nb_fur}_{algo}_{version}.json", "r"))
    ...
    extract_best_worst_scn(diff_dist[idx], fur_number, diff_between=list_vers, alg=list_algo, version=list_vers)
...
def extract_best_worst_scn(diff_data, nb_fur, diff_between, alg, version):
    ...
    print(f"Parameters : number_furniture: {nb_fur} | algorith: {alg} | version: {version}")
...
def number_scen(nb_fur, algo, vers):
    data = json.load(open(
            f"new_seed_gen/distance_nb{nb_fur}_{algo}_{vers}.json", "r"))
    nb_folds = len(data["agent_0"]["total_dist"])
    return nb_folds
...
def plot_bar(data, label):
    ...
    plt.bar(br1, drag, color ='r', width = barWidth,
        edgecolor ='grey', label =label[0])
    plt.bar(br2, nodrag, color ='g', width = barWidth,
        edgecolor ='grey', label =label[1])

    plt.xlabel('Number of furnitures', fontweight ='bold', fontsize = 15)
    plt.ylabel('Converged scenario[%]', fontweight ='bold', fontsize = 15)
    plt.xticks([r + barWidth for r in range(len(drag))],
            [2,3,5,6,7,8])
