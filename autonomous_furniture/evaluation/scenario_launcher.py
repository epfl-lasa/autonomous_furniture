...
class ScenarioLauncher:
    def __init__(self, nb_furniture=5, record=False):
        ...
        self._init_index_free_space = [(i, j) for i in range(
            self._resolution[0]) for j in range(self._resolution[1])]
        self._init_index_occupied_space = []

        self._goal_index_free_space = [(i, j) for i in range(
            self._resolution[0]) for j in range(self._resolution[1])]
    ...
    def place_agent_randomly(self, free_space: list = None, occupied_space: list = None) -> ObjectPose:
        ...
        if True:  # while is_placed is not True:
            ...
            for idx in grid_coord:
                ...
                if self._fur_shape.is_inside(position, margin=1):
    ...
    def setup(self):
        ...
        for ii in range(self._nb_furniture):
            ...
            self.agents.append(Furniture(shape=self._fur_shape, obstacle_environment=self.obstacle_environment,
                               control_points=np.array([[0.4, 0], [-0.4, 0]]), goal_pose=self._goal_setup[ii]))
