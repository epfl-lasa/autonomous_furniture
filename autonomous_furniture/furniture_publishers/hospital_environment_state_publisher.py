...
class DynamicalSystemRviz(Node):
    ...
    def update_state_publisher(self, prefix, position, rotation):
        ...
        self.odom_trans.transform.rotation = \
            euler_to_quaternion(pi/2, 0, rotation)  # rpy
        if "qolo" in prefix:
    ...
    def run(
        ...
        furniture_attractor_avoider = FurnitureAttractorDynamics(furniture_env, cutoff_distance=2)
        position_list = []
        velocity_list = []
        ...
        for ii, furniture in enumerate(furniture_env):
            if furniture.num_control_points > 0:
                position_list[ii][:, :, 0] = furniture.relative2global(furniture.rel_ctl_pts_pos,
        ...
        while ii < it_max:
            ...
            for index, furniture in enumerate(furniture_env):
                if furniture.furniture_type == "person":
                    u_obs_vel = furniture.furniture_container.linear_velocity / np.linalg.norm(
