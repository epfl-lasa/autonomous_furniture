...
class Grid:
    def __init__(self, x_lim, y_lim, furnitures,resolution = [20,20]) -> None:
        ...
        self._step_y = (max(y_lim)-min(y_lim))/resolution[1] # Defining the step along x and y to scan the grid / y is a row in python
        self.area_obj_dict = {}
    ...
    def create_grid(self, x_lim, y_lim, resolution):
        for ii in range(resolution[0]):
            for jj in range(resolution[1]):
                self._grid[ii,jj] = [(min(x_lim)+ ii*self._step_x + self._step_x/2 ), (min(y_lim) + jj*self._step_y) + self._step_y/2 ] # we store the middle coordinate of each cell in the grid list
