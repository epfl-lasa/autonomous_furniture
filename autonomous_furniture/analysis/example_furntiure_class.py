    ...
    def run(
        ...
        for obs in range(num_obs):
            if furniture_env[obs].is_controllable:
                relative_agent_pos[obs_w_multi_agent[obs], :] = furniture_env[
                    obs
                ].rel_ctl_pts_pos
                pass
        ...
        while ii < it_max:
            ...
            weights = furniture_avoider.get_influence_weight_at_points(
            ...
            for jj, furniture in enumerate(furniture_env):
                if furniture.is_controllable:
                    ...
                    global_attractor_pos = furniture.relative2global(
                        relative_attractor_position, furniture.goal_container
                    )
                    attractor_vel = np.zeros((num_attractor, dim))
                    for attractor in range(num_attractor):
                        attractor_vel[attractor, :] = attractor_dynamic.evaluate(
                            global_attractor_pos[attractor, :]
                        )
                    attractor_weights = attractor_dynamic.get_weights_attractors(
                        global_attractor_pos
                    )
                    goal_vel, goal_rot = attractor_dynamic.get_goal_velocity(
                        global_attractor_pos, attractor_vel, attractor_weights
                    )
                    new_goal_pos = (
                        goal_vel * dt_step + furniture.goal_container.center_position
                    )
                    new_goal_ori = (
                        -goal_rot * dt_step + furniture.goal_container.orientation
                    )
                    ...
                    for i in obs_w_multi_agent[jj]:
                        furniture_avoider.set_attractor_position(
                            global_attractor_pos[i], i
                        )
            ...
            for obs in range(num_obs):
                ...
                for agent in obs_w_multi_agent[obs]:
                    ...
                    velocity[agent, :] = furniture_avoider.evaluate_furniture(
...
def multiple_robots():
    ...
    max_ax_len = max(axis)
    min_ax_len = min(axis)
    obstacle_pos = np.array([[center_point, 0.0], [3.0, 0.0]])
