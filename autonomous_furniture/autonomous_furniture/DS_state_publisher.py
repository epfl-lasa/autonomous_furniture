...
class DynamicalSystemRviz(Node):
    ...
    def run(
        ...
        while ii < it_max:
            ...
            if ii % 10 == 0:
                for jj in goal:
                    attractor_vel = attractor_dynamic.evaluate(jj.center_position)
                    new_goal_pos = attractor_vel * dt_step + jj.center_position
                    jj.center_position = new_goal_pos
                    global_attractor_pos = relative2global(relative_attractor_position, jj)
            ...
            for obs in range(num_obs):
                ...
                for agent in obs_w_multi_agent[obs]:
                    ...
                    print(f"obs num: {obs}")
                    print(f"agent num: {agent}")
                    print(f"current agent pos: {position_list[agent, :, ii - 1]}")
...
def main():
    ...
    obstacle_environment.append(Cuboid(
        axes_length=[max_ax_len, min_ax_len],
        center_position=np.array([-1, -2]),  # obstacle_pos[1]
        margin_absolut=0,
        orientation=math.pi/2,
        tail_effect=False,
        repulsion_coeff=1,
